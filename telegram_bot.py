...
def format_automation_report(reading: dict, report: dict, plant_name: str, pump_seconds: float) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            f"Bitki: {plant_name}",
            f"Sicaklik: {float(reading['temperature_c']):.2f} C",
            f"LM35 ham ortalama: {float(reading['raw_avg']):.1f}",
            f"LM35 ham aralik: {int(reading['raw_min'])}-{int(reading['raw_max'])}",
            f"LM35 voltaj: {float(reading['voltage']):.4f} V",
...
def build_system_report(db_path: Path, plant_name: str = "cherry tomato") -> str:
    ...
    lines = [
        ...
        f"Sicaklik: {float(row['temperature_c']):.2f} C",
        f"LM35 ham ortalama: {float(row['raw_avg']):.1f}",
        f"LM35 ham aralik: {int(row['raw_min'])}-{int(row['raw_max'])}",
        f"LM35 voltaj: {float(row['voltage']):.4f} V",
